...
def test_persistent_memory(tmp_path):
    ...
    memory.record_failure("API missing key exception when booting", "Set your API key using export")
    memory.record_failure("Failed to install numpy version 1.2", "Use pip install numpy==1.24 explicitly")
    ...
    results = memory.get_relevant_failures("I am trying to install numpy but it fails continuously")
    assert len(results) >= 1
    assert "pip install numpy" in results[0]["correction"]
    
    # Test completely irrelevant retrieval (should return empty)
    results_irrelevant = memory.get_relevant_failures("Where is the database located in the folder?")
